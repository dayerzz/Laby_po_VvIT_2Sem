...
app = Flask(__name__)
conn = psycopg2.connect(database="service_db", user="postgres", password="11RinDer", host="localhost", port="5432")
...
def login():
    if request.method == 'POST':
        if request.form.get('login'):
            ...
            if not records:
                return render_template('login.html', error="Пользователя с такими даннами не существует. Проверьте свой"
                                                           "логин и пароль!")
            print(records)
...
def registration():
    if request.method == 'POST':
        ...
        password = request.form.get('password')
        if not name:
            return render_template('registration.html', error="Пожалуйста, введите данные о себе!")
        if not log1n:
